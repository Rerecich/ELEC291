...
int main(void)
{
	...
	while (1)
	{
		...
		capacitance = ((1.44*T)/6000)*1000000; //to account for microfarads
		capacitancenf = ((1.44*T)/6000)*1000000000; // nano farads
		
		printf("PB2 (pin 16)=%c\r", (PINB&0b00000100)?'1':'0');
		//_delay_ms(300);
		...
		else if(!(PINC&0b00000010||!(PINC&0b00000100))){
		sprintf(str, "%f Hz", f);
		LCDprint(str,2,1);
		}
		else{
		sprintf(str, "%f nF", capacitancenf);
		LCDprint(str,2,1);
		}

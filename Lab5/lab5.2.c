...
void Period_Frequency_ADC(void)
{
	...
	half_period=TH0*256.0+TL0; // The 16-bit number [TH0-TL0], 256 = 16^2
	
	period = 2*half_period; //pretty sure this can be accessed globally
...
//correct inputs
   ...
   else if (strcmp(input,"reference voltage")==0) //if test voltage
   {
       DisplayReferenceVoltage:
       LCDprint("ReferenceVoltage:     \n",1,1);
       while(1) {
           // Read 14-bit value from the pins configured as analog inputs
	    // Read 14-bit value from the pins configured as analog inputs
	    v[0] = Volts_at_Pin(REFERENCE);
		vrms2 = v[0]/1.414;
           sprintf(str, "%10.7f", vrms2);
           LCDprint(str, 2,1);
       }
   ...
   else if (strcmp(input,"phase shift")==0) 
  {
       ...
       while(1) {
			...
			while(Get_ADC()<150); // Wait for the signal to be one
			...
			TR0 = 1;
			while ((overflow_count*65536.0+TH0*256.0+TL0)*(12.0/SYSCLK)<(period/4))
			{
			...
			TR0=0;
			vrms2 = Volts_at_Pin(REFERENCE)/1.414;
		...
		// Start tracking the reference signal
		ADC0MX=TEST;//ADC0MX=QFP32_MUX_P2_5;
		ADINT = 0;
		ADBUSY=1;
		while (!ADINT); // Wait for conversion to complete
		TL0=0; 
		...
		while(Get_ADC()>50); // Wait for the signal to be one
		while(Get_ADC()<50);  // Wait for the signal to be zero
		...
		phasediff = (overflow_count*65536.0+TH0*256.0+TL0)*(12.0/SYSCLK)*(360.0/(period));
       
       		sprintf(str, "%10.7f", phasediff);
			//sprintf(str,"%10.7f", period);

			printf("period:  %5.5f  phase shift:  %5.5f\r", period, phasediff);
			//LCDprint(":            ",1,1);
       		LCDprint(str, 2,1);
       }
...
//spelling mistakes
    ...
    {
            printf(RED"Did you mean phase shift? [Y/N]");
            getsn(input, sizeof(input));
            ...
            printf(RED"Did you mean reference voltage? [Y/N]");

...
void Peak_Voltage(void) 
{
	...
	while (Get_ADC()<50); // Wait for the signal to be positive
...
void main (void)
{
    ...
    while(1) {
        ...
        printf("test: %2.3f V || ref: %2.3f V|| period: %2.3f s|| phase shift: %3.3f\r", vrmsTest, vrmsRef, period, phase_difference);
...
//automatic adjustments ; look up edit distance

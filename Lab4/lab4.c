...
void main (void) 
{
	...
	while((PB_1 ^ PB_2) == 1) //wait for release
	{
		if(PB_1 == 0){
			...
			printf("Enter units: uF [1] or F [2]: ");
			//get units input 
			getsn(input, sizeof(input));
			printf("\n");
			//check units input
			if (strcmp(check1,input) == 0) //if it's 1
			{
				...
				while (1)
    			{
					...
					percent_error = ((capacitance - cap_input_int)/(cap_input_int));
			...
			else if (strcmp(check2,input) == 0) //if it's 2 (or technically anything else but shh)
			{
			...
			{
				LCDprint("Capacitance (uF) error:",1,1);
				//calculate capacitance afterwards cuz it doesnt work if i do it before
				while (1)
    			{
